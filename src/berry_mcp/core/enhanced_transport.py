...
from ..auth import OAuth2Manager, AuthenticationMiddleware
...
class EnhancedSSETransport(SSETransport):
    ...
    def __init__(
        ...
        self.elicitation_manager: Optional[ElicitationManager] = None
    ...
    async def connect(self) -> None:
        ...
        if not self.app:
            raise RuntimeError("EnhancedSSETransport requires an assigned FastAPI app instance")
        
        logger.info("EnhancedSSETransport: Configuring routes with OAuth2 and elicitation support")
        
        # Setup authentication if required
        auth_scheme = HTTPBearer(auto_error=self.require_auth) if FASTAPI_AVAILABLE else None
        
        # Add enhanced routes
        if FASTAPI_AVAILABLE:
            # Main endpoints with optional authentication
            if self.require_auth and auth_scheme:
                self.app.post("/")(self._create_authenticated_handler(self._handle_message))
                self.app.post("/message")(self._create_authenticated_handler(self._handle_message))
            else:
            ...
            if self.elicitation_manager:
                self.app.post("/elicitation/response")(self._handle_elicitation_response)
                self.app.get("/elicitation/active")(self._handle_list_active_prompts)
        
        logger.info(f"EnhancedSSETransport: Ready with authentication={'enabled' if self.require_auth else 'disabled'}")
    ...
    def _create_authenticated_handler(self, handler):
        ...
        async def authenticated_handler(request: Request, background_tasks: BackgroundTasks):
    ...
    async def _handle_health(self, request: Request = None) -> Any:
        ...
        if self.elicitation_manager:
            health_info["active_prompts"] = len(self.elicitation_manager.get_active_prompts())
    ...
    def get_token_info(self) -> Optional[Any]:
