...
class MCPProtocol:
    ...
    async def handle_message(self, message_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if message_data.get("jsonrpc") != "2.0":
            logger.warning(f"Invalid JSON-RPC version in message: {str(message_data)[:150]}")
        ...
        try:
            ...
            logger.debug(f"Handler for '{method}' completed successfully, returned: {type(result_data)}")
            ...
            logger.error(f"Exception during handler execution for '{method}' (ID: {request_id}): {detailed_error}", exc_info=True)
    ...
    def _format_result(self, req_id: Optional[str | int], result: Any) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Result for request ID {req_id} is not JSON serializable: {e}")
            final_result = f"[Non-Serializable Result: {type(result).__name__}] {str(result)[:500]}"
        except Exception as e:
            logger.error(f"Unexpected error during result serialization for ID {req_id}: {e}")
        ...
        if data is not None:
            ...
            if isinstance(data, (str, int, float, bool, list, dict, type(None))):
                ...
                try:
                    ...
                    logger.warning(f"Error data contains non-standard type {type(data).__name__}")
