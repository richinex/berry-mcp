...
try:
    ...
    class EventSourceResponse: pass
...
class StdioTransport(Transport):
    ...
    async def connect(self):
        ...
        try:
            ...
            logger.error(f"StdioTransport: Failed to connect to stdin: {e}", exc_info=True)
            self.closed = True
            return

        self._stdin_task = asyncio.create_task(self._read_stdin_async(), name="StdioReader")
    ...
    async def _read_stdin_async(self):
        ...
        while not self.closed:
            try:
                ...
                logger.error(f"StdioTransport: Unexpected error in reader: {e}", exc_info=True)
                self.closed = True
                await self._receive_queue.put(None)
                break
...
class SSETransport(Transport):
    ...
    def __init__(self, host: str = "localhost", port: int = 8000):
        if not FASTAPI_AVAILABLE:
            raise ImportError("FastAPI and related dependencies required for SSETransport")
    ...
    async def _handle_sse(self, request: Request):
        ...
        async def event_generator():
            ...
            try:
                ...
                yield {
                    "event": "system",
                    "data": json.dumps({"type": "connected", "message": "SSE connection established"}),
                ...
                while not self.closed:
                    try:
                        ...
                        message_dict = await asyncio.wait_for(client_queue.get(), timeout=15.0)
    ...
    async def _run_handler_background(self, request_data: Dict[str, Any]):
        ...
        if not self._message_handler:
            logger.error(f"No message handler configured for background execution")
            error_resp = {
                "jsonrpc": "2.0",
        ...
        try:
            ...
            if isinstance(response_data, dict):
                ...
                ack_msg = {
                    "jsonrpc": "2.0",
                    "id": request_id,
                    "result": {"status": "processed", "message": f"Background task completed"}
                }
            ...
            logger.error(f"Background execution failed for {method}: {e}", exc_info=True)
            error_resp = {
                "jsonrpc": "2.0",
                "error": {"code": -32000, "message": f"Background execution failed: {str(e)}"},
                "id": request_id
            }
            await self.send(error_resp)
    ...
    async def _handle_message(self, request: Request, background_tasks: BackgroundTasks):
        ...
        try:
            ...
            if not isinstance(request_data, dict) or request_data.get("jsonrpc") != "2.0":
                return JSONResponse(
                    status_code=400,
                    content={"error": "Invalid JSON-RPC structure"}
                )
            ...
            if not method:
                return JSONResponse(
                    status_code=400,
                    content={"error": "Missing method parameter"}
                )
            ...
            # Handle tools/call in background for immediate response
                ...
                if isinstance(response_data, dict):
                    ...
                    return JSONResponse(
                        status_code=500,
                        content={"error": "Invalid handler response"}
                    )
    ...
    async def send(self, message: Dict[str, Any]) -> None:
        ...
        for client_queue in list(self.clients):
            try:
                ...
                logger.warning(f"Timeout sending to SSE client, skipping message {track_id}")
            except Exception as e:
