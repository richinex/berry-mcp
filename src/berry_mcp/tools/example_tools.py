...
def generate_uuid(version: int = 4) -> Union[str, Dict[str, str]]:
...
) -> Union[str, int, float, List, Dict[str, str]]:
    """
    ...
    try:
        if data_type == "string":
            ...
            if charset not in charsets:
                return {"error": f"Unknown charset: {charset}. Available: {list(charsets.keys())}"}
...
def hash_text(text: str, algorithm: str = "sha256") -> Union[str, Dict[str, str]]:
    """
    ...
    if algorithm not in algorithms:
        return {"error": f"Unknown algorithm: {algorithm}. Available: {list(algorithms.keys())}"}
