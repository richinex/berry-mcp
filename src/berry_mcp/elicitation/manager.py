...
from .handlers import ElicitationHandler, ConsoleElicitationHandler, SSEElicitationHandler  
...
class ElicitationManager:
    ...
    def __init__(
        self, 
        handler: Optional[ElicitationHandler] = None,
    ...
    async def handle_response(self, prompt_id: str, response: Any) -> None:
        ...
        if isinstance(self.handler, SSEElicitationHandler):
            ...
            logger.warning(f"Cannot handle response for prompt {prompt_id} - handler doesn't support it")
    ...
    def create_enhanced_tool_info(self, tool_func: Callable, capability: CapabilityMetadata) -> dict[str, Any]:
...
class StreamingResultManager:
    ...
    async def start_stream(self, operation_id: str, tool_name: str, metadata: dict[str, Any] = None) -> None:
    ...
    async def complete_stream(
        ...
        if error:
            ...
            logger.info(f"Completed streaming operation: {operation_id} ({duration:.2f}s, {stream_info['chunks_sent']} chunks)")
