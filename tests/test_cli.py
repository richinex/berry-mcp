...
async def test_run_stdio_server():
    ...
    with patch('berry_mcp.server.MCPServer') as mock_server_class, \
...
def test_cli_main_http():
    ...
    test_args = ['berry-mcp', '--transport', 'http', '--host', '0.0.0.0', '--port', '9000']
...
def test_server_name_from_env():
    ...
    with patch.dict(os.environ, {'BERRY_MCP_SERVER_NAME': 'env-server'}), \
...
async def test_server_error_handling():
    ...
    with patch('berry_mcp.server.MCPServer', side_effect=Exception("Server error")), \
         patch('berry_mcp.server.setup_logging'):
